...
RULES = [
    ...
    ("즉/정리하면", r"(^|[\s(“\"])(즉(?![시각결])|정리하면|다시 말해|말하자면|요약하면)", PROSE_LIKE),
    ("이것이/그것이", r"(?<![가-힣])(이것이|그것이|이게|그게)(\s|$)", PROSE_LIKE),
    ("비유",
     r"안전벨트|타임머신|리모컨|세이브 ?포인트|보스전|건축주|현장 감독|도면|엔진입니다|열쇠|전기·수도|기반 설비|과외 선생님|"
    ...
    ("구어체 훅",
     r"혹시 [^.?]{0,40}\?|(?<![임줄준])말입니다\.|있으신가요|아니신가요|지요\.|하지요|이지요|되지요|거죠|것이죠|건데요|"
...
REF = {"em대시(산문)": 0.0, "볼드(산문)": 0.0, "문장형 괄호": 0.0, "긴 괄호(18자+)": 0.0, "첫째/둘째": 0.0,
       "즉/정리하면": 0.0, "이것이/그것이": 0.0, "구어체 죠": 0.0, "입니다 종결 비중%": 10.81,
...
def profile(rows):
    ...
    return {
        ...
        "즉/정리하면": p1k(r"(^|[\s(])(즉(?![시각결])|정리하면|다시 말해|말하자면)"),

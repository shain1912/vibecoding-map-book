...
- 본문에는 `그림 6.1 — 설치 전 node -v 실행 결과`처럼 캡션 문단(Caption 스타일)으로 자리를 잡아 두었습니다.
- 4장~29장은 그림 파일 이름이 캡션의 번호와 같습니다. 예: 캡션 `그림 6.1` -> 파일 `그림6.1.png`
- 1장~3장은 `그림1-1_편익과_윤리적_문제.png`처럼 이름에 설명이 붙은 것과 `그림3.1.png` 형식이 섞여 있습니다.
  이 세 장은 공저자 원고라 번호 체계를 그대로 두었습니다. `그림목록.csv`의 「수록 장」으로 확인해 주세요.
- 화면을 그대로 담은 것이라 확대 없이 쓰시면 됩니다. 가장 큰 것이 2400×1500 픽셀이고,
  작은 것은 320×180 픽셀까지 있습니다.
...
- 상자로 넣을 부분은 첫 줄의 표시로 구분합니다. `💡 팁`, `⚠️ 주의`, `🗣 이렇게 시키세요`,
  `이 장에서 배우는 것`, `다음 장 예고` 가 그것이며, 지금은 모두 본문(Normal) 스타일입니다.
- 각주는 본문에 위첨자 숫자로 표시하고, 그 내용은 해당 장 끝에 번호 목록으로 두었습니다.
